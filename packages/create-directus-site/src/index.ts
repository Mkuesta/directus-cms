...
async function main() {
  ...
  const root = path.resolve(process.cwd(), opts.siteSlug);

  if (fs.existsSync(root)) {
    console.error(`\nError: Directory "${opts.siteSlug}" already exists.`);
    process.exit(1);
  }

  console.log(`\nScaffolding ${opts.siteName} in ./${opts.siteSlug}...\n`);
...
`);
  ...
  if (opts.includeNewsletter) {
    write(path.join(root, 'lib', 'newsletter.ts'), gen.generateNewsletterConfig(opts));
    write(path.join(root, 'app', 'api', 'newsletter', 'route.ts'), gen.generateNewsletterApiRoute());
  }
  if (opts.includeNotifications) {
    write(path.join(root, 'lib', 'notifications.ts'), gen.generateNotificationsConfig(opts));
  }
  ...
  console.log('\n✅ Done! Next steps:\n');
  console.log(`  cd ${opts.siteSlug}`);
  console.log('  npm install --legacy-peer-deps');
  console.log('  # Update .env.local with your Directus token');
