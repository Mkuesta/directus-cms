...
export function checkHoneypot(config: FormConfig, honeypotValue: string | undefined): boolean {
  if (!(config.honeypotEnabled ?? true)) return true;
  // If honeypot field has a value, it's a bot
  return !honeypotValue;
}

export function checkRateLimit(config: FormConfig, ip: string | undefined): boolean {
  if (!ip) return true;
